...
FORCE_BANK_FROM_GLOBAL_IDX = {
    # chapter_id: (global_idx_from, forced_bank)
    6: (3963, 1), # chapter 6: from idx 3963 => force bank 1 (as a safe switch used by default)
}
...
def on_export():
    ...
    for scene_id, pos in enumerate(pointer_tables_pos):
        ...
        if not chapter_fields:
            ...
            # Chapter 6 one-time export deduplication (bank 0 only)
            # - If a same .text appears multiple times while still in bank 0,
            #   only the first occurrence keeps its text bytes.
            # - Next occurrences point back to the first one, and we only write
            #   a single 0x20 byte (space) to keep the cursor moving.
            # NOTE: This intentionally makes the exported .RES not reliably
            #       re-importable by this script (one-time export feature).
            dedup_bank0 = (chapter_id == 6)
            dedup_ptr_by_text = {}  # text -> pointer16 (bank0)
            ...
            for field in chapter_fields_sorted:
                forced = FORCE_BANK_FROM_GLOBAL_IDX.get(chapter_id)
                bank_min = int(field.get("bank", 0) or 0)
                if forced and field["idx"] >= forced[0]:
                    bank_min = max(bank_min, forced[1])

                # Always treat missing text as empty string
                field_text = field.get("text", "") or ""

                # ----------------------------------------------------------
                # Chapter 6 export-only deduplication (bank 0 only)
                # ----------------------------------------------------------
                if dedup_bank0 and bank_min == 0 and field_text in dedup_ptr_by_text:
                    placeholder = b"\x20"  # ' ' (space)
                    bank = 0
                    while True:
                        ...
                        pointer16_tmp = cursor - bank_base  # offset inside current bank (0..0xFFFF)

                        # If the placeholder would cross bank boundary -> promote bank (and disable dedup)
                        if pointer16_tmp <= 0xFFFF and (pointer16_tmp + len(placeholder)) <= 0x10000:
                            break

                        bank += 1

                    # Only apply dedup while we're still in bank 0
                    if bank == 0:
                        field["_export_bank"] = 0
                        pointer16 = dedup_ptr_by_text[field_text]  # go backwards to first occurrence

                        # Write placeholder (unreferenced; keeps cursor moving forward)
                        if cursor == len(buf):
                            buf.extend(placeholder)
                        else:
                            if cursor > len(buf):
                                buf.extend(b"\x00" * (cursor - len(buf)))
                            buf[cursor:cursor + len(placeholder)] = placeholder
                ...
                text_bytes = field_text.encode(ENCODING) + b"\x00"
                ...
                # Record first occurrence for bank 0 dedup (chapter 6 only)
                if dedup_bank0 and bank == 0 and field_text not in dedup_ptr_by_text:
                    dedup_ptr_by_text[field_text] = pointer16
